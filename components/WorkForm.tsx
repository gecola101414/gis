...
const WorkForm: React.FC<WorkFormProps> = ({ idvs = [], orders = [], existingChapters = [], currentUser, onSubmit, onCancel, initialData, prefilledChapter }) => {
  // FILTRO: L'utente può vedere solo i capitoli a lui assegnati
  const myIdvs = useMemo(() => {
    if (currentUser.role === UserRole.ADMIN) return idvs;
    return idvs.filter(i => i.assignedToId === currentUser.id || i.ownerId === currentUser.id);
  }, [idvs, currentUser]);

  const myChapters = useMemo(() => Array.from(new Set(myIdvs.map(i => i.capitolo))), [myIdvs]);
  ...
  const filteredIdvs = useMemo(() => {
    return myIdvs.filter(i => i.capitolo === selectedChapter);
  }, [myIdvs, selectedChapter]);
  ...
  const newExpectedResidual = chapterResidual - (formData.estimatedValue || 0);
  const isOutOfBudget = newExpectedResidual < 0;

  return (
    <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl border border-slate-100 max-w-4xl mx-auto animate-in fade-in duration-500">
      <div className="mb-8 border-b border-slate-50 pb-6">
        <h2 className="text-3xl font-black text-slate-800 tracking-tighter italic">Pianificazione Progetto</h2>
        <p className="text-slate-400 text-xs font-bold mt-1 uppercase tracking-widest italic">Puoi caricare lavori solo sui capitoli a te assegnati.</p>
      </div>
      
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} className="space-y-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">Capitolo Autorizzato</label>
            <select 
              value={selectedChapter}
              onChange={(e) => { setSelectedChapter(e.target.value); setFormData({ ...formData, linkedIdvIds: [] }); }}
              className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl outline-none font-bold focus:border-indigo-600"
            >
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">Valore Stimato (€)</label>
            <VoiceInput
              type="number"
              value={formData.estimatedValue || ''}
              onChange={(v) => setFormData({ ...formData, estimatedValue: Number(v) })}
              placeholder="Importo..."
              className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl font-black text-indigo-600 text-lg focus:border-indigo-600"
            />
          </div>

          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">Status Residuo</label>
            <div className={`w-full px-5 py-4 border-2 rounded-2xl flex flex-col justify-center ${isOutOfBudget ? 'bg-rose-50 border-rose-500 text-rose-600' : 'bg-emerald-50 border-emerald-500 text-emerald-600'}`}>
              <p className="text-lg font-black leading-none">€{newExpectedResidual.toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2">
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">Descrizione Intervento</label>
            <VoiceInput
              type="textarea"
              value={formData.description || ''}
              onChange={(v) => setFormData({ ...formData, description: v })}
              placeholder="Dettagli tecnici..."
              className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl font-medium focus:border-indigo-600"
            />
          </div>
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">Allegato Progetto</label>
            ...
            <label htmlFor="pdf-upload" className={`w-full h-[108px] border-2 border-dashed rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all ${formData.projectPdf ? 'border-emerald-200 bg-emerald-50 text-emerald-600' : 'border-slate-200 bg-slate-50 text-slate-400 hover:border-indigo-300'}`}>
              <span className="text-[10px] font-black uppercase text-center px-4 truncate w-full">{formData.projectPdf ? formData.projectPdf.name : 'Carica PDF'}</span>
            </label>
          </div>
        </div>
        ...
        {selectedChapter && (
          <div className="animate-in fade-in slide-in-from-bottom-4">
            <label className="block text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-4 italic">Seleziona Sorgente Fondo (Cap. {selectedChapter})</label>
        ...
        <div className="flex justify-end gap-5 pt-8 border-t border-slate-100">
          <button type="button" onClick={onCancel} className="px-8 py-4 font-bold text-slate-400 hover:text-slate-600 transition-colors uppercase text-[10px] tracking-widest">Annulla</button>
          <button type="submit" disabled={!selectedChapter || !formData.linkedIdvIds?.length} className={`px-12 py-4 text-white font-black rounded-2xl shadow-xl transition-all ${(!selectedChapter || !formData.linkedIdvIds?.length) ? 'bg-slate-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
            Registra Progetto
          </button>
        </div>
      </form>
